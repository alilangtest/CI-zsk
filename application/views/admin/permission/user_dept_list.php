...
//     var unodes=utreeObj.getCheckedNodes(true);
    ...
    if(operids == ""){
    	if(confirm("未设置任何操作权限，是否保存？")){
    		$.post("/admin/user_rights/save_data","deptid="+deptid+"&userid="+userid+"&operids="+operids,function(data){
    	        if(data=="ok"){
    	        	initUserTree(deptid);
    	        	layer.alert('保存成功！');
    	        }
...
function initUserTree(deptid){
	$.post("/admin/user_rights/get_users","id="+deptid,function(data){
...
function initCheckbox(){
	...
	$.post("/admin/user_rights/get_operation","deptid="+deptid+"&userid="+userid,function(data){
...
<div id="content">
	...
	<div class="content_right">
		<div>
			<input id="keyword" type="text" onchange="change();" style="height:25px;width:230px;margin:3px 0 0 5px;" placeholder="请输入...">
            <input type="button" onclick="search();" class="layui-btn layui-btn-sm" value="搜索"  />
        </div>
    	<ul id="utree" class="ztree" style="height:650px;" ></ul>
    </div>
	<div id="operation" style="float:left;width:550px;" >
		<div>
			<form class="layui-form" lay-filter="cbForm"  action="">
            	<div class="layui-form-item" pane="" style="clear:both;" >
                    <label class="layui-form-label" style="width:45px;" >权限：</label>
                    <div class="layui-input-block" style="margin-left:10px;">
                    	<input type="checkbox"  name="oper" value="1"  lay-skin="primary" title="移动">
                        <input type="checkbox"  name="oper" value="2"  lay-skin="primary" title="删除">
                        <input type="checkbox"  name="oper" value="3"  lay-skin="primary" title="审核">
                        <input type="checkbox"  name="oper" value="4"  lay-skin="primary" title="分享">
                        <input type="checkbox"  name="oper" value="5"  lay-skin="primary" title="重命名">
                        <input type="checkbox"  name="oper" value="6"  lay-skin="primary" title="回收站">
                    </div>
                </div>
            </form>
		</div>
		<div style="clear:both;">
        	<input type="button" onclick="save();" class="layui-btn" value="保存"  style="margin:20px 0 0 100px;" />
        </div>
	</div>
</div>
</body>
