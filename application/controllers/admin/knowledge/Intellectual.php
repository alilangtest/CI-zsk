 ...
 * 干啥滴：知识库列表
 */
class Intellectual extends MY_Controller
{
    const COLUMN_NAME = "我的知识";
    ...
    //知识库列表()
    public function intellectual_list()
    {
         //查询该用户所属部门
        ...
        if($kw && $kw != ''){
            $where = "articles.status = 1 and (articles.title like '%$kw%' or different.tname like '%$kw%') ";
        }
        //组合条件
        if(isset($where)){
            $where .= " AND articles.ding_userid = ".$this->user->ding_userid." AND articles.dpt = -1 AND articles.department = ".$_SESSION['default_dept'];
        }else{
            $where = "articles.ding_userid = ".$this->user->ding_userid." AND articles.status = 1  AND  articles.dpt = -1 AND articles.department = ".$_SESSION['default_dept'];
        }
        ...
        //每一页显示的数据条数的变量
        $page_size=9;
        $config['base_url']=site_url("admin/knowledge/intellectual/intellectual_list/");
        ...
        $this->load->view('/admin/knowledge/intellectual_list',$data);
    }
     ...
     * */
    public function intellectual_add(){
...
//                print_r($this->input->post());die;
                ...
                $this->article->ding_userid = $this->user->ding_userid; //关联用户信息改用ding_userid
                ...
                $this->article->dpt = -1;//dpt = -1 表示是个人知识
                ...
                if($stt == false){
                  log_message('error','新增个人知识日志写入失败!');
                }
            ...
            $data['col'] = $arr;
            $this->load->view('/admin/knowledge/intellectual_add',$data);
        }
    }

    //删除操作
    public function intellectual_del(){
        //删除个人的知识 没有必须判断权限!
    ...
    //修改记录
    public function intellectual_update($id = ''){
        $this->load->model('different/Article_model','article',true);
        if($this->input->post()){
//            print_r($this->input->post());die;
            //服务端进行表单验证
...
//                print_r($this->input->post());die;
                ...
                if($stt == false){
                  log_message('error','修改个人知识日志写入失败!');
                }
            ...
            $this->load->view('/admin/knowledge/intellectual_update',$data);
        }

    }

    //知识分享弹出页面
    public function intellectual_share(){
        //要分享的文章的id集合；
        ...
        $data['depts'] = $depts;
        $data['artcle_ids'] = $ids;
        $this->load->view('/admin/knowledge/intellectual_share',$data);
    }
    ...
    public function do_share(){
        ...
        foreach($ars as $v){
            foreach($dis as $v1 ){
                ...
                // $this->article_share->uid = $this->user->id;//分享人为当前登录用户 hsl
                $this->article_share->ding_userid = $this->user->ding_userid; //分享人为当前登录用户 hsl
                ...
                // //验证该文章是否已经分享过 如果分享过那么就提示错误信息
                // $num = $this->article_share->verify_share_dpt($v,$v1);
                // if($num>0){
                //     exit(json_encode(array('status'=>0,'msg'=>'二次分享!将更新之前的分享操作!')));
                // }
                $this->article->retrieve($v);
                ...
                if($st){
                  ...
                  if($stt == false){
                    log_message('error','分享个人知识日志写入失败!');
                  }
                }
            }
